  ...
  async function onSubmit(values) {
    ...
    try {
      ...
      if (result.exists) {
        ...
        navigate(`/dashboard?rollNumber=${values.rollNumber}`)
      } else {
        toast.warning("Face not registered. Redirecting to register.")
        navigate("/register")
      }
    } catch (error) {
    ...
    <div className="flex flex-col items-center justify-start px-4 py-6 min-h-screen lg:py-0 bg-gray-50 overflow-x-hidden sm:justify-center">
      ...
      <Card className="w-full max-w-md shadow-lg bg-white">
        <CardHeader className="flex flex-row items-center justify-between">
          <CardTitle className="text-xl md:text-2xl">Sign in to your account</CardTitle>
          <AdminButton />
        </CardHeader>
        <CardContent>
          <Form {...form}>
